...
Returns process statistic information..

> note: for server wide statistics (not restricted to the current session) use the method `server.stats()`.
...
try {
    const sessions = await mind.session.stats()
...
const stats = await mind.session.stats()
console.log(stats)
